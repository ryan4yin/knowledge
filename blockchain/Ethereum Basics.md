...
首先我们要明确一个事实：「我们不可能把所有数据都存放在区块链上」，原因有很多，最明显的就是相比 MySQL/Redis 等中心化数据库、以及 clickhouse 等数据湖而言，区块链的交易实在太慢了，完全无法满足需求。另外不同的数据库有不同的特性——海量的存储与查询分析、向量搜索、关系查询、地理位置搜索等等。这导致我们的大部分数据肯定仍然是需要存放在这些外部数据库中的，这也是为什么需要「Oracles」的原因。

理想情况下，预言机是无需信任的系统，这意味着它们不需要被信任，因为它们按照去中心化原则运行。
如果预言机本身是中心化的，那运行在区块链上的智能合约显然就会有获取不到数据而执行失败的风险。

一个去中心化的预言机网络（Decentralized Oracle Networks, DON）...

预言机需要验证从外部提供者获取到的数据，确认其是高质量的、真实无误的，所以也被称为 validator.
