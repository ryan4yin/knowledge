...
6. **分析性能压测结果报告**：分析解读性能压测结果报告，判定性能压测是否达到预期目标，若不满足，要基于性能压测结果报告分析原因
    - 链路追踪可以记录下每次请求期间的各种 RPC/Redis/MySQL 等各种调用的耗时、甚至自定义指标，是性能分析的好帮手。
...
而并发用户数，指导意义不大。另一方面如果为了模拟真实用户行为而引入用户等待时间（比如 Locust 的 wait_time），这时并发用户数其实就很难统计了。
