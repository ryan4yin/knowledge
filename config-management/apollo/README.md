...
它功能强大，支持配置的继承，也有提供 API 方便自动化配置。
缺点是权限管理比较弱，也不支持信息加密，不适合直接存储敏感信息。
...
Apollo 也提供了简单的 HTTP API 鉴权的功能，在添加并启用了访问密钥后，Apollo 将要求客户端通过密钥访问 API，否则无法获取配置。

这个访问密钥是「环境」级别的，一个环境下的所有集群都共享同一套密钥配置，它们的权限全部相同！（这和 vautl 的权限策略相比，可弱多了）
