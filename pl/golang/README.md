...
- 可选的第二个返回值 `ok`
  ...
  - 如果调用方没接收第二个返回值 `ok`，那失败的调用将会导致 panic
  - 如果调用方接收了第二个返回值 `ok`，这个值将用于表示调用是否成功。并且不会触发 panic
  - 在 Go 1.18 之前，只有官方的内置类型才提供「可选的第二个返回值 `ok`」这样的能力
  - 从 Go 1.18 开始，用户可以通过泛型在自定义的类型上实现这个功能。
